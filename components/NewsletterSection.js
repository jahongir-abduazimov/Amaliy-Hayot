...
export default function NewsletterSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStatus('error');
        setMessage(data.error || 'Xatolik yuz berdi. Iltimos, keyinroq urinib ko\'ring.');
      }
    } catch (error) {
      setStatus('error');
  ...
  return (
    <section id="newsletter-section" className="bg-linear-to-br from-blue-50 to-cyan-50 py-12 sm:py-16 md:py-20">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        {/* Status Message */}
        {status && message && (
          <div className={`mt-4 px-4 py-3 rounded-xl max-w-md mx-auto ${status === 'success'
