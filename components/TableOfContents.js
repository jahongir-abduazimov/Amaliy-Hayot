...
export default function TableOfContents({ content }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (observer) {
        const headingElements = document.querySelectorAll('article .prose h1, article .prose h2, article .prose h3');
        headingElements.forEach((heading) => {
      ...
      <nav>
        <ul className="space-y-2">
          {headings.map((heading) => (
            <li key={heading.id} className={`text-sm ${heading.level === 2 ? 'pl-4' : heading.level === 3 ? 'pl-6' : 'pl-0'}`}>
              <a
                href={`#${heading.id}`}
                onClick={(e) => {
                  e.preventDefault();
