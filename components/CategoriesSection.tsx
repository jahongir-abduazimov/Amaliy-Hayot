...
export default function CategoriesSection({ posts = [] }: CategoriesSectionProps) {
  ...
  const categories = [
    { name: 'Hujjatlar', icon: '📄', color: 'blue', count: categoriesMap['Hujjatlar'] || 0 },
    { name: 'Moliya', icon: '💰', color: 'emerald', count: categoriesMap['Moliya'] || 0 },
    { name: 'Xizmatlar', icon: '🏛️', color: 'purple', count: categoriesMap['Xizmatlar'] || 0 },
          ...
          {categories.map((category) => {
            ...
            return (
              ...
              >
                ...
                {category.name}
                {category.count > 0 && (
                  <span className={`ml-2 ${isActive ? 'opacity-90' : 'opacity-70'}`}>
                    ({category.count})
                  </span>
                )}
              </button>
